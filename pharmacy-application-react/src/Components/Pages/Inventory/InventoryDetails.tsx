...
function InventoryDetails() {
  ...
  const [inventoryInvoiceAmount, setInventoryInvoiceAmount] =
    useState<number>(0.0);
    ...
    if (params && params["id"]) {
      axios
        ...
        .then((res) => {
          ...
          for (let item of inventoryResponse.inventoryDetails) {
            const cgstAmt = parseFloat(
              String(
                (Number(item.totalManufacturerRate) * Number(item.cgst)) / 100,
              ),
            ).toFixed(2);
            const sgstAmt = parseFloat(
              String(
                (Number(item.totalManufacturerRate) * Number(item.sgst)) / 100,
              ),
            ).toFixed(2);
            const invoiceAmt = parseFloat(
              String(
                Number(item.totalManufacturerRate) +
                  Number(cgstAmt) +
                  Number(sgstAmt),
              ),
            ).toFixed(2);

            item.invoiceAmount = Number(invoiceAmt);
          }
          setInventoryInvoiceAmount(
            inventoryResponse.inventoryDetails.reduce(
              (a, b) => Number(a) + Number(b.invoiceAmount),
              0,
            ),
          );
  ...
  const calculateNetAmt = (
    ...
    setDataSource((prevState) => {
      ...
      setInventoryInvoiceAmount(
        state.reduce((a, b) => Number(a) + Number(b.invoiceAmount), 0),
      );
      return state;
    });
  };
  ...
  return (
    <div className="inventory">
      ...
      <InventoryParent
        inventory={inventory}
        setInventory={updateFields}
        inventoryInvoiceAmount={inventoryInvoiceAmount}
      />
